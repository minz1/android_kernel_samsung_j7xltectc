	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	int offset;
	char *dec_num;
#if defined (CONFIG_MIC_TDMA_NOISE)
	int micbias_register_check;
#endif
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		snd_soc_update_bits(codec,
				...
				0xFF, 0x42);
#if defined (CONFIG_MIC_TDMA_NOISE)
		micbias_register_check = snd_soc_read(codec, MSM8X16_WCD_A_ANALOG_MICB_1_EN);
		if (micbias_register_check & 0x80)
			snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_MICB_1_INT_RBIAS, 0x29, 0x29);
		micbias_register_check = snd_soc_read(codec, MSM8X16_WCD_A_ANALOG_MICB_2_EN);
		if (micbias_register_check & 0x80)
			snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_MICB_1_INT_RBIAS, 0x18, 0x08);
#endif
		break;
	case SND_SOC_DAPM_POST_PMU:
		/* enable HPF */
	...
	case SND_SOC_DAPM_POST_PMD:
#if defined (CONFIG_MIC_TDMA_NOISE)
		snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_MICB_1_INT_RBIAS, 0xFF, 0x00);
#endif
