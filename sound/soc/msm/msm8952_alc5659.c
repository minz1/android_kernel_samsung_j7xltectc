...
#define DRV_NAME "msm8952-asoc-alc5659"
...
int check_ext_spk_gpio_support(struct platform_device *pdev,
...
int msm_q6_enable_mi2s_clocks(bool enable, int port_id)
{
...
static int quin_mi2s_sclk_ctl(struct snd_pcm_substream *substream, bool enable)
{
	...
	if (enable) {
		...
		if ((substream->stream == SNDRV_PCM_STREAM_CAPTURE)
			&& (pdata->ext_pa & QUIN_MI2S_ID))
			msm_q6_enable_mi2s_clocks(true, AFE_PORT_ID_QUINARY_MI2S_TX);
#endif /* CONFIG_AUDIO_SPEAKER_OUT_NXP_AMP_ENABLE */
	} else {
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
#ifdef CONFIG_AUDIO_SPEAKER_OUT_NXP_AMP_ENABLE
		...
		if ((substream->stream == SNDRV_PCM_STREAM_CAPTURE)
			&& (pdata->ext_pa & QUIN_MI2S_ID))
			msm_q6_enable_mi2s_clocks(false, AFE_PORT_ID_QUINARY_MI2S_TX);
#endif
			mi2s_tx_clk.clk_val1 = Q6AFE_LPASS_IBIT_CLK_DISABLE;
			ret = afe_set_lpass_clock(
					AFE_PORT_ID_QUINARY_MI2S_TX,
...
static int rt5659_start_sysclk(struct snd_soc_card *card)
{
	struct msm8916_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
	int ret;
...
static int rt5659_stop_sysclk(struct snd_soc_card *card)
{
	struct msm8916_asoc_mach_data *pdata = snd_soc_card_get_drvdata(card);
...
static int rt5659_set_bias_level(struct snd_soc_card *card,
				struct snd_soc_dapm_context *dapm,
				enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		if (card->dapm.bias_level == SND_SOC_BIAS_PREPARE)
			rt5659_stop_sysclk(card);
		break;
	case SND_SOC_BIAS_PREPARE:
		if (card->dapm.bias_level == SND_SOC_BIAS_STANDBY)
			rt5659_start_sysclk(card);
		break;
	default:
		break;
	}
...
void msm8952_alc5659_disable_mclk(struct work_struct *work)
{
	struct msm8916_asoc_mach_data *pdata = NULL;
...
static int msm8952_alc5659_asoc_machine_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card;
	struct msm8916_asoc_mach_data *pdata = NULL;
	...
	pdata->codec_ldo_1p8_en = of_get_named_gpio(pdev->dev.of_node,
								"qcom,codec_ldo_1p8_en", 0);
	pdata->codec_ldo_3p3_en = of_get_named_gpio(pdev->dev.of_node,
								"qcom,codec_ldo_3p3_en", 0);

	if (gpio_is_valid(pdata->codec_ldo_1p8_en)) {
		if (gpio_request(pdata->codec_ldo_1p8_en, "codec_ldo_1p8_en"))
			dev_err(&pdev->dev, "Failed to request codec_ldo_1p8_en\n");
		else if (gpio_direction_output(pdata->codec_ldo_1p8_en, 1))
			dev_err(&pdev->dev, "Failed to set codec_ldo_1p8_en\n");
	}

	if (gpio_is_valid(pdata->codec_ldo_3p3_en)) {
		if (gpio_request(pdata->codec_ldo_3p3_en, "codec_ldo_3p3_en"))
			dev_err(&pdev->dev, "Failed to request codec_ldo_3p3_en\n");
		else if (gpio_direction_output(pdata->codec_ldo_3p3_en, 1))
			dev_err(&pdev->dev, "Failed to set codec_ldo_3p3_en\n");
	}
...
#endif /* CONFIG_SAMSUNG_JACK */
	ret = check_ext_spk_gpio_support(pdev, pdata);
	if (ret < 0)
	...
	if (ret)
		goto err;
	/* initialize timer */
	INIT_DELAYED_WORK(&pdata->disable_mclk_work, msm8952_alc5659_disable_mclk);
	...
	pdata->codec = card->rtd[36].codec;	// added rt5659 codec
	...
	if (!of_find_property(pdev->dev.of_node, "clock-names", NULL)) {
		...
		pdata->rt5659_ext_clk = clk_get(card->dev, "rt5659_clk");
...
static int msm8952_alc5659_asoc_machine_remove(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(pdata->codec_ldo_1p8_en))
		gpio_free(pdata->codec_ldo_1p8_en);
	if (gpio_is_valid(pdata->codec_ldo_3p3_en))
		gpio_free(pdata->codec_ldo_3p3_en);
	if (gpio_is_valid(pdata->codec_irq_gpio))
...
static const struct of_device_id msm8952_alc5659_asoc_machine_of_match[]  = {
	{ .compatible = "qcom,msm8952-alc5659-audio-codec", },
	{},
};

static struct platform_driver msm8952_alc5659_asoc_machine_driver = {
	.driver = {
		.name = DRV_NAME,
		.owner = THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = msm8952_alc5659_asoc_machine_of_match,
	},
	.probe = msm8952_alc5659_asoc_machine_probe,
	.remove = msm8952_alc5659_asoc_machine_remove,
};
module_platform_driver(msm8952_alc5659_asoc_machine_driver);
...
MODULE_DEVICE_TABLE(of, msm8952_alc5659_asoc_machine_of_match);
