...
static int voice_send_dha_data(struct voice_data *v)
{
	...
	pr_info("%s: send vpcm_dha_param_send_cmd, mode = %d, select=%d\n",
		__func__, mode, vpcm_dha_param_send_cmd.dha_send.select);
	...
	pr_debug("%s: wait for vpcm_dha_param_send_cmd\n", __func__);
