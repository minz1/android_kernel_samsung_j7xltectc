 ...
 * capacity.
 */
#ifndef CONFIG_SCHED_DYNAMIC_THRESHOLD
...
#ifdef CONFIG_SCHED_DYNAMIC_THRESHOLD
	/*
	 * Traverse the task list and set the upmigrate threshold for all
	 * tasks. Since this is _not_ a frequent operation hence the 
	 * additional penalty is acceptable.
	 */
	read_lock(&tasklist_lock);
	do_each_thread(g, p) {
		p->se.upmigrate_threshold = up_migrate;
...
#ifdef CONFIG_SCHED_DYNAMIC_THRESHOLD
	return load > p->se.upmigrate_threshold;
#else
...
#ifdef CONFIG_SCHED_DYNAMIC_THRESHOLD
		upmigrate = p->se.upmigrate_threshold;
#else
...
#endif /* CONFIG_SCHED_TASK_BEHAVIOR */
#ifdef CONFIG_SCHED_DYNAMIC_THRESHOLD
	p->se.upmigrate_threshold = pct_to_real(sysctl_sched_upmigrate_pct);
#endif /* CONFIG_SCHED_DYNAMIC_THRESHOLD */
}
