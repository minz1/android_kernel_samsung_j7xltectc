			 ...
			 u32 runtime, int samples, int event)
{
	...
	u64 sum = 0;
#ifdef CONFIG_SCHED_DYNAMIC_THRESHOLD
	u32 is_increasing = 1;
#endif /* CONFIG_SCHED_DYNAMIC_THRESHOLD */
...
#ifdef CONFIG_SCHED_DYNAMIC_THRESHOLD
	/*
	 * Figure out whether the task has an increasing execution history
	 */

	for (widx = 0; widx < sched_ravg_hist_size - 1; widx++) {
		if(hist[widx] <= hist[widx+1]) {
			is_increasing = 0;
			break;
		}
	}
	...
	/*
	 * Dynamically change the per-task upmigrate threshold to set a higher
	 * value for the non-increasing tasks. Such that those tasks are not
	 * immediately migrated to the big cluster.
	 */

	if((p->on_rq) && (p->sched_class == &fair_sched_class) && !p->se.my_q && !&p->se) {
		dec_hmp_sched_stats_fair(rq, p);
		if(!is_increasing) {
			p->se.upmigrate_threshold =
				pct_to_real(sysctl_sched_upmigrate_pct) +
				pct_to_real(5);
			trace_printk("The upmigrate for the task is: %llu", real_to_pct(p->se.upmigrate_threshold));
		} else {
			p->se.upmigrate_threshold =
				pct_to_real(sysctl_sched_upmigrate_pct);
		}
		inc_hmp_sched_stats_fair(rq, p);
	}
#endif /* CONFIG_SCHED_DYNAMIC_THRESHOLD */
