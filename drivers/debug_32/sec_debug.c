...
static int __init __init_sec_debug_log(void)
{
	...
	pr_info("%s: vaddr=0x%lx paddr=0x%llx size=0x%x "\
		...
		(unsigned long)sizeof(struct sec_debug_log));
...
{
    struct sec_debug_summary_data_modem *modem = (struct sec_debug_summary_data_modem *)&secdbg_summary->priv.modem;
...
static int __init sec_dt_addr_init(void)
{
	...
	if (!restart_reason) {
 	...
 	pr_info("%s: sizeof(unsigned int) = %d sizeof(unsigned int64_t) = %d\n",__func__,sizeof(unsigned int),sizeof(uint64_t));
...
void sec_do_bypass_sdi_execution_in_low(void)
{
	...
	if (ret)
		...
		pr_info("%s: Disabled wdog debug for debug level LOW\n", __func__);
}
...
static ssize_t sec_user_fault_write(struct file *file, const char __user *buffer,
		size_t count, loff_t *offs)
{
	char buf[100];
