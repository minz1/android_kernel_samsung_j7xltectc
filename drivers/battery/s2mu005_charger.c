				...
				enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		psy_do_property("s2mu005-charger", set, POWER_SUPPLY_PROP_CHARGE_OTG_CONTROL, value);
		break;
	default:
		return -EINVAL;
	}
...
static void s2mu005_test_read(struct i2c_client *i2c)
{
	...
	s2mu005_read_reg(i2c, 0x20, &data); // hiden register
	sprintf(str+strlen(str), "0x%02x:0x%02x, ", 0x20, data);
...
static bool s2mu005_chg_init(struct s2mu005_charger_data *charger)
{
	...
	s2mu005_write_reg(charger->client, 0x1A, 0x82); // watch-dog timer enable
	s2mu005_update_reg(charger->client, 0x2A, 1 << 3, 0x08); // set async time 20msec recover
...
static void s2mu005_charger_initialize(struct s2mu005_charger_data *charger)
{
	...
	s2mu005_write_reg(charger->client, 0x1A, 0x82);
...
static irqreturn_t s2mu005_event_isr(int irq, void *data)
{
	struct s2mu005_charger_data *charger = data;
	/* Delay 100ms for debounce */

	pr_info("%s \n", __func__);
	wake_lock_timeout(&charger->vbus_wake_lock, 200);
...
static irqreturn_t s2mu005_vbus_isr(int irq, void *data)
{
	struct s2mu005_charger_data *charger = data;
	/* Delay 100ms for debounce */
...
static int s2mu005_charger_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&charger->vbus_wake_lock, WAKE_LOCK_SUSPEND,
			"vbus_wake_lock");
	...
	/* this code is for evt2 */
	if(charger->dev_id >= 2) {
		charger->irq_event = pdata->irq_base + S2MU005_CHG_IRQ_EVENT;
		ret = request_threaded_irq(charger->irq_event, NULL,
				s2mu005_event_isr, 0 , "event-irq", charger);
		if(ret < 0) {
			dev_err(s2mu005->dev, "%s: Fail to request event-irq in IRQ: %d: %d\n",
						__func__, charger->irq_event, ret);
			goto err_reg_irq;
		}

		charger->irq_vbus = pdata->irq_base + S2MU005_CHG_IRQ_VBUS;
		ret = request_threaded_irq(charger->irq_vbus, NULL,
				s2mu005_vbus_isr, 0 , "vbus-irq", charger);
		if(ret < 0) {
			dev_err(s2mu005->dev, "%s: Fail to request vbus-irq IRQ: %d: %d\n",
						__func__, charger->irq_vbus, ret);
			goto err_reg_irq;
		}
	}
