...
int static s2mu005_set_adc_curroffset(struct s2mu005_fuelgauge_data *fuelgauge)
{
	u8 data[2];
	...
	"%s: coffset_old:0x%x, coffset_new:0x%x, coffset_old1:(%d)mA, coffset_new1:(%d)mA\n",
	__func__, coffset_old, coffset_new,coffset_old1, coffset_new1);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		if (val->intval == SEC_FUELGAUGE_CAPACITY_TYPE_RAW) {
			...
			if (fuelgauge->revision < 2) {
				if((fuelgauge->is_charging == true) && (val->intval >= 100) && (fuelgauge->coffset_flag == false)) {
					fuelgauge->coffset_flag = true;
					if(s2mu005_set_adc_curroffset(fuelgauge) < 0)
						return -EINVAL;
				}
				else if((fuelgauge->is_charging == false) && (fuelgauge->coffset_flag == true)) {
					fuelgauge->coffset_flag = false;
					data[0] = (fuelgauge->coffset_old & 0x00FF);
				    data[1] = (fuelgauge->coffset_old & 0xFF00) >> 8;
					s2mu005_write_reg(fuelgauge->i2c, S2MU005_REG_COFFSET, data);
					dev_info(&fuelgauge->i2c->dev, "%s: change coffset to original value\n", __func__);
				}
			}
