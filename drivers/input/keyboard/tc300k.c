...
static void tc300k_reset_work(struct work_struct *work)
{
	struct tc300k_data *data =
			container_of(work, struct tc300k_data, reset_work.work);
...
static irqreturn_t tc300k_interrupt(int irq, void *dev_id)
{
	...
	if (ret < 0) {
		...
		if (retry <= 0) {
			schedule_delayed_work(&data->reset_work, msecs_to_jiffies(10));
			return IRQ_HANDLED;
		}
	}
	key_val = (u8)ret;
		...
		const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&data->reset_work, tc300k_reset_work);
