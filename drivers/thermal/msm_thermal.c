		...
		long temp)
{
	...
	if (ret) {
		...
		, cluster_ptr ? cluster_ptr->cluster_id : 0, freq_table[idx].frequency,
		temp, ret);
		return;
	}
	...
	pr_debug("Sensor:%s Limiting Cluster%d max frequency to %u. Temp:%ld\n",
		prog->sensor_info->name, cluster_ptr ? cluster_ptr->cluster_id : 0,
...
#ifndef CONFIG_SEC_DISABLE_KTM_IN_LOW_TEMP
	if ((vdd_rstr_enabled) &&
