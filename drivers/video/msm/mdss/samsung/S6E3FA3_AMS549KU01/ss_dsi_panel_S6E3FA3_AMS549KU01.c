...
static int mdss_hbm_read(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (vdd->dtsi_data[ctrl->ndx].elvss_rx_cmds[vdd->panel_revision].cmd_cnt) {
		/* Read mtp (B6h 22th) for hbm elvss */
		mdss_samsung_panel_data_read(ctrl,
...
static struct dsi_panel_cmds *mdss_hbm_gamma(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	...
	pr_info("%s vdd->auto_brightness : %d\n", __func__, vdd->auto_brightness);
...
static struct dsi_panel_cmds * mdss_elvss_temperature2(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	...
	if (vdd->temperature > 0) {
		vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = vdd->display_status_dsi[ctrl->ndx].elvss_value;
	} else if (vdd->temperature <= 0 && vdd->temperature > -15) {
		if (candela_value >= 5 && candela_value <= 20)		/* 5 ~ 20 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x1B;
		else if (candela_value == 21)				/* 21 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x19;
		else if (candela_value == 22)				/* 22 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x18;
		else if (candela_value == 24)				/* 24 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x17;
		else if (candela_value == 25)				/* 25 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x16;
		else if (candela_value == 27)				/* 27 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x15;
		else if (candela_value == 29)				/* 29 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x14;
		else if (candela_value == 30)				/* 30 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x13;
		else
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = vdd->display_status_dsi[ctrl->ndx].elvss_value;
	} else {
		if (candela_value >= 5 && candela_value <= 20)		/* 5 ~ 20 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x1F;
		else if (candela_value == 21)				/* 21 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x1E;
		else if (candela_value == 22)				/* 22 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x1D;
		else if (candela_value == 24)				/* 24 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x1C;
		else if (candela_value == 25)				/* 25 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x1A;
		else if (candela_value == 27)				/* 27 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x18;
		else if (candela_value == 29)				/* 29 nit */
			vdd->dtsi_data[ctrl->ndx].elvss_lowtemp2_tx_cmds[vdd->panel_revision].cmds[1].payload[1] = 0x16;
...
static void mdss_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_get_conf_hmt = smart_get_conf_S6E3FA3_AMS549KU01_hmt;
