...
int mdss_samsung_hbm_brightenss_packet_set(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (vdd->display_status_dsi[ndx].hbm_mode && !vdd->auto_brightness_level) {
		pr_err("%s DSI%d: already hbm mode! return \n", __func__, ndx);
		return 0;
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if ((input > vdd->auto_brightness_level) && (vdd->auto_brightness_level !=0) ) {
		pr_info("%s: Invalid argument!! input(%d), auto_brightness_level(%d)\n", __func__, input, vdd->auto_brightness_level);
		return size;
	}
...
void mdss_samsung_dsi_dump_regs(int dsi_num)
{
	...
	if (vdd_data.panel_attach_status & BIT(dsi_num)) {
		...
		if (dsi_ctrl[dsi_num]->shared_ctrl_data->phy_regulator_io.base) {
